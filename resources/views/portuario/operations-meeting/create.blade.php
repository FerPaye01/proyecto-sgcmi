...
@section('title', 'Nueva Junta de Operaciones')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-sgcmi-blue-900">Nueva Junta de Operaciones</h1>
        <p class="text-gray-600 mt-2">Registre los acuerdos y programación de operaciones para las próximas 24 horas</p>
    </div>
    
    <div class="card">
        <form method="POST" action="{{ route('operations-meeting.store') }}" x-data="meetingForm()">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea id="agreements" 
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sgcmi-blue-500 focus:border-transparent @error('agreements') border-red-500 @enderror">{{ old('agreements') }}</textarea>
                @error('agreements')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t">
                ...
                <button type="submit" class="btn-primary">
                    Registrar Junta de Operaciones
                </button>
            </div>
        </form>
    </div>
...
function meetingForm() {
    return {
        ...
        scheduleItems: [
            { vessel: '', operation: '', start_time: '', estimated_duration_h: '' }
        ],
