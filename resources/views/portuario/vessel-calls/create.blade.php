...
@section('title', 'Crear Llamada de Nave')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Crear Llamada de Nave</h2>
        ...
        <form action="{{ route('vessel-calls.store') }}" 
              method="POST" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="estado_llamada" 
                        ...
                        <option value="EN_CURSO" {{ old('estado_llamada') == 'EN_CURSO' ? 'selected' : '' }}>En Curso</option>
                        <option value="COMPLETADA" {{ old('estado_llamada') == 'COMPLETADA' ? 'selected' : '' }}>Completada</option>
                        <option value="CANCELADA" {{ old('estado_llamada') == 'CANCELADA' ? 'selected' : '' }}>Cancelada</option>
                    </select>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="motivo_demora" 
                              ...
                              placeholder="Describa el motivo de la demora si aplica">{{ old('motivo_demora') }}</textarea>
                    @error('motivo_demora')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
